...
import { makeBike, makeCar, makeCrate, makeHuman, makeRifle, makeSniper, disposeSharedMats } from "./models";
import {
  BIKE_ACCEL,
  BIKE_BRAKE,
  BIKE_DRAG,
...
import { buildWorld, groundHeightAt, rayAabb, resolveCircle, separateCircles, type WorldData } from "./world";
...
type Vehicle = {
  ...
  occupied: boolean;
  kind: VehicleKind;
  // Ambient traffic: a subset of parked cars slowly patrol between world
  // waypoints on their own when nobody's in them, so the city doesn't feel
  // like every vehicle is a static prop — and since entering one just sets
  // `occupied`, driving takes over exactly like any other car once you do.
  traffic: boolean;
  wp: number;
};
...
export class GameEngine {
  ...
  private player = {
    ...
    yaw: 0.2,
    pitch: -0.08,
    // Separate from `yaw` (camera/aim direction). This is what the visible
    // body actually faces — it turns toward whichever way the player is
    // moving, the way a real person (or a GTA-style character) does,
    // instead of the body staying locked to camera-facing while sliding
    // sideways during a strafe.
    facingYaw: 0.2,
    hp: MAX_HP,
    ammo: MAG_SIZE,
    reserve: 240,
  ...
  constructor(canvas: HTMLCanvasElement, onHud?: (h: GameHud) => void) {
    ...
    this.pColor = new Float32Array(pCount * 3).fill(1);
  ...
  private onNet(from: string, data: unknown, _ch: "state" | "reliable") {
    ...
    if (t === "s") {
      ...
      if (bot && bot.hp > 0) {
        ...
        if (bot.hp <= 0) {
          this.hitstop = 0.045;
          this.bloodBurst(bot.x, 1, bot.z, 22);
          bot.respawn = 7;
  ...
  private spawnVehicles() {
    ...
    const bikeColors = [0x1a1c22, 0xe0392b, 0x2f6fd6, 0xffb020];
    this.world.carSpawns.forEach((s, i) => {
      // Every 4th spawn point gets a bike instead of a car, so the two
      // vehicle types are mixed through the city rather than segregated.
      const isBike = i % 4 === 3;
      const mesh = isBike ? makeBike(bikeColors[i % bikeColors.length]) : makeCar(colors[i % colors.length]);
      mesh.position.set(s.x, 0, s.z);
      mesh.rotation.y = s.yaw;
      this.scene.add(mesh);
      const traffic = i % 3 === 0;
      let wp = 0;
      if (traffic && this.world.waypoints.length > 0) {
        let best = 0;
        let bestD = Infinity;
        this.world.waypoints.forEach((w, wi) => {
  ...
  private step(dt: number) {
    ...
    if (this.p2p && this.netAcc >= 0.05) {
      this.netAcc = 0;
      // While driving, a car's body should match its actual heading (which
      // player.yaw already tracks via updateDrive), same as before. On
      // foot, broadcast facingYaw (movement-direction facing) instead of
      // the raw camera/aim yaw, so other players see your character turn
      // to face where you're actually walking rather than sliding
      // sideways while facing wherever your camera happens to point.
      const bodyYaw = this.player.inCar >= 0 ? this.player.yaw : this.player.facingYaw;
      const msg: NetState = {
        t: "s",
        x: +this.player.x.toFixed(2),
        y: +this.player.y.toFixed(2),
        z: +this.player.z.toFixed(2),
        yaw: +bodyYaw.toFixed(3),
  ...
  private updateFoot(dt: number, act: Actions) {
    ...
    p.z += nz * spd * dt;
    if (len > 0.08) {
      // Face the direction actually being moved in, not the camera/aim
      // direction — matches how a real person (or GTA) turns their body
      // toward travel rather than sliding sideways while still facing
      // forward. Smoothly turned rather than snapped so it doesn't look
      // like a sudden jump cut.
      const targetFacing = Math.atan2(-nx, -nz);
      p.facingYaw = lerpAngle(p.facingYaw, targetFacing, 1 - Math.exp(-14 * dt));
    }
    ...
    p.vy -= GRAVITY * dt;
    p.y += p.vy * dt;
    // Rest on whatever surface (street level, a stair tread, a rooftop) is
    // actually under the player's feet, instead of always clamping to a
    // flat y=0 — this is what makes stairs and rooftops standable at all.
    const ground = groundHeightAt(p.x, p.z, p.y, this.world.walkSurfaces);
    if (p.y <= ground) {
      p.y = ground;
      p.vy = 0;
      p.onGround = true;
  ...
  private updateDrive(dt: number, act: Actions) {
    ...
    const isBike = car.kind === "bike";
    const vMax = isBike ? BIKE_MAX : CAR_MAX;
    const vAccel = isBike ? BIKE_ACCEL : CAR_ACCEL;
    const vBrake = isBike ? BIKE_BRAKE : CAR_BRAKE;
    const vTurn = isBike ? BIKE_TURN : CAR_TURN;
    const vDrag = isBike ? BIKE_DRAG : CAR_DRAG;
    const vRadius = isBike ? BIKE_RADIUS : CAR_RADIUS;
    let steer = act.steer;
    if (steer > 1) steer = 1;
    if (steer < -1) steer = -1;
    const throttle = act.throttle;
    if (throttle > 0.05) car.speed += throttle * vAccel * dt;
    else if (throttle < -0.05) car.speed += throttle * vBrake * dt;
    else car.speed *= 1 - vDrag * 1.8 * dt;
    car.speed *= 1 - vDrag * dt;
    car.speed = Math.max(-vMax * 0.45, Math.min(vMax, car.speed));
    const speedFactor = Math.max(0, Math.min(1, Math.abs(car.speed) / 8));
    const reverse = car.speed >= 0 ? 1 : -1;
    car.yaw += steer * vTurn * speedFactor * reverse * dt;
    const fx = -Math.sin(car.yaw);
    const fz = -Math.cos(car.yaw);
    car.x += fx * car.speed * dt;
    car.z += fz * car.speed * dt;
    const res = resolveCircle(car.x, car.z, vRadius, this.world.colliders);
    ...
    this.player.z = car.z;
    this.player.y = 0.2;
    // player.yaw is NOT synced to car.yaw here anymore — it's driven purely
    // by look input (mouse drag / touch drag / gamepad right stick), same
    // as on foot, so you can aim independently of the direction you're
    // driving (a proper drive-by), instead of the camera and crosshair
    // being welded to whichever way the car happens to be pointed.
  ...
  // Ambient traffic: unoccupied, traffic-flagged cars slowly patrol between
  // world waypoints on their own, the same way bots patrol on foot — makes
  // the city feel populated instead of every parked car being a static
  // prop, and any of them can still be entered/driven normally at any time
  // (entering just flips `occupied`, which stops this update for that car).
  private updateTraffic(dt: number) {
    for (const car of this.vehicles) {
      if (car.occupied || !car.traffic) continue;
      if (this.world.waypoints.length === 0) continue;
      const wp = this.world.waypoints[car.wp];
      const dist = Math.hypot(wp.x - car.x, wp.z - car.z);
      const want = Math.atan2(-(wp.x - car.x), -(wp.z - car.z));
      car.yaw = lerpAngle(car.yaw, want, 1 - Math.exp(-2.2 * dt));
      if (dist < 4) {
        car.wp = (car.wp + 1) % this.world.waypoints.length;
      } else {
        const fx = -Math.sin(car.yaw);
        const fz = -Math.cos(car.yaw);
        const targetSpeed = 4.2;
        car.speed += (targetSpeed - car.speed) * Math.min(1, dt * 1.5);
        const nx = car.x + fx * car.speed * dt;
        const nz = car.z + fz * car.speed * dt;
        const vRadius = car.kind === "bike" ? BIKE_RADIUS : CAR_RADIUS;
        const res = resolveCircle(nx, nz, vRadius, this.world.colliders);
        car.x = res.x;
        car.z = res.z;
      }
      car.mesh.position.set(car.x, 0, car.z);
  ...
  private tryEnterExit() {
    if (this.player.inCar >= 0) {
      ...
      car.occupied = false;
      car.speed = 0;
      // Step out well clear of the vehicle's own collision radius (car or
      // bike — smaller for a bike, so exiting doesn't fling you an
      // unnecessarily large distance away), so the exit camera's near
      // plane never ends up inside the vehicle mesh (which renders as a
      // blank screen with only the camera-attached gun visible, since
      // everything else falls behind the near clip / inside solid
      // geometry).
      const vRadius = car.kind === "bike" ? BIKE_RADIUS : CAR_RADIUS;
      const clearance = vRadius + PLAYER_RADIUS + 1.2;
      ...
      for (const [cx, cz] of candidates) {
        ...
        const clearOfCar = Math.hypot(res.x - car.x, res.z - car.z) >= vRadius + PLAYER_RADIUS + 0.1;
      ...
      this.player.y = 0;
      this.player.inCar = -1;
      // Reset look direction to face the way the car was heading when you
      // step out, rather than wherever you'd been aiming while driving —
      // a sane, predictable starting view on foot instead of an arbitrary
      // leftover drive-by aim angle.
      this.player.yaw = car.yaw;
      this.player.facingYaw = car.yaw;
      this.player.pitch = 0;
      this.audio.carDoor();
      return;
    }
    ...
    if (hitBot && local) {
      ...
      if (this.isHost) {
        ...
        if (hitBot.hp <= 0) {
          ...
          this.hitstop = 0.045;
          this.trauma = Math.min(1, this.trauma + 0.28);
          this.bloodBurst(hitBot.x, 1, hitBot.z, 22);
          hitBot.respawn = 7;
        }
      } else {
  ...
  private updateGrenades(dt: number) {
    for (const g of this.grenades) {
      ...
      if (g.y < 0.12) {
        // Explode immediately on ground contact instead of waiting out a
        // fuse timer — grenades still bounce off walls/geometry mid-air via
        // the wall-collision branch above, but touching the ground is the
        // detonation trigger now, matching how a real impact/frag grenade
        // (rather than a timed one) behaves.
        g.y = 0.12;
  ...
  private respawn() {
    ...
    this.player.facingYaw = this.player.yaw;
  }
  ...
  // GTA-style body looting: walk up to a bot you've killed (while its body
  // is still lying there, before it respawns) to pick up a small amount of
  // ammo and cash — a smaller, opportunistic reward compared to a dedicated
  // supply crate, and each body can only be looted once.
  // People, cars, and other characters were only ever checked against
  // static building/tree geometry, never against each other — so they
  // could freely stand or drive on top of one another. This pushes
  // overlapping pairs apart, using the same circle math as world
  // collision, once per frame after everyone's normal movement resolves.
  private separateEntities() {
    // Player vs. bots (skip dead/looted bodies lying on the ground — those
    // should still be walkable-over, matching how a corpse works in most
    // shooters, not a solid obstacle).
    if (this.player.inCar < 0) {
      for (const b of this.bots) {
        if (b.hp <= 0) continue;
        const r = separateCircles(this.player.x, this.player.z, PLAYER_RADIUS, 1, b.x, b.z, 0.4, 1);
        this.player.x = r.ax;
        this.player.z = r.az;
        b.x = r.bx;
        b.z = r.bz;
      }
      // Player vs. remote players — remotes are authoritative on their own
      // client, so only the LOCAL side of the pair is actually kept; this
      // just stops you from standing inside someone else, it doesn't try
      // to shove them (their next network update would overwrite that).
      for (const rp of this.remotes.values()) {
        if (rp.hp <= 0 || rp.car >= 0) continue;
        const r = separateCircles(this.player.x, this.player.z, PLAYER_RADIUS, 1, rp.x, rp.z, 0.4, 1000);
        this.player.x = r.ax;
        this.player.z = r.az;
      }
    }
    // Player vs. unoccupied vehicles (occupied ones — including the one
    // you're currently in — are handled by the normal drive/enter logic).
    for (const car of this.vehicles) {
      if (car.occupied) continue;
      const carR = car.kind === "bike" ? BIKE_RADIUS : CAR_RADIUS;
      const inCarVehicle = this.player.inCar >= 0 ? this.vehicles[this.player.inCar] : undefined;
      const pr = inCarVehicle ? (inCarVehicle.kind === "bike" ? BIKE_RADIUS : CAR_RADIUS) : PLAYER_RADIUS;
      const px = inCarVehicle ? inCarVehicle.x : this.player.x;
      const pz = inCarVehicle ? inCarVehicle.z : this.player.z;
      if (px === undefined || pz === undefined) continue;
      const r = separateCircles(px, pz, pr, 3, car.x, car.z, carR, 1);
      if (this.player.inCar >= 0) {
        this.vehicles[this.player.inCar].x = r.ax;
        this.vehicles[this.player.inCar].z = r.az;
      } else {
        this.player.x = r.ax;
        this.player.z = r.az;
      }
    ...
    // Bots vs. each other, and bots vs. unoccupied vehicles — both are
    // simulated locally (bots are host-authoritative but every client runs
    // this the same way against its own synced copy), so pushing both
    // sides apart is safe here.
    for (let i = 0; i < this.bots.length; i++) {
      ...
      for (let j = i + 1; j < this.bots.length; j++) {
        const b = this.bots[j];
        if (b.hp <= 0) continue;
        const r = separateCircles(a.x, a.z, 0.4, 1, b.x, b.z, 0.4, 1);
        a.x = r.ax;
        a.z = r.az;
        b.x = r.bx;
        b.z = r.bz;
      ...
      for (const car of this.vehicles) {
        if (car.occupied) continue;
        const carR = car.kind === "bike" ? BIKE_RADIUS : CAR_RADIUS;
        const r = separateCircles(a.x, a.z, 0.4, 1, car.x, car.z, carR, 4);
        a.x = r.ax;
        a.z = r.az;
        car.x = r.bx;
        car.z = r.bz;
  ...
  private collectLoot() {
    for (const b of this.bots) {
      if (b.hp > 0 || b.looted || !b.mesh.visible) continue;
      if (Math.hypot(b.x - this.player.x, b.z - this.player.z) > 2.2) continue;
      b.looted = true;
      this.audio.pickup();
      this.player.reserve = Math.min(360, this.player.reserve + 20);
      if (Math.random() < 0.35) this.player.bombs = Math.min(16, this.player.bombs + 1);
      this.player.cash += 15 + Math.floor(Math.random() * 26);
    }
  }
  ...
  private updateBots(dt: number, target: { x: number; z: number; y: number } | null) {
    for (const b of this.bots) {
      if (b.hp <= 0) {
        b.respawn -= dt;
        // Leave the body visible, lying down, for most of the respawn
        // window instead of vanishing the instant they die — that's the
        // window during which it can be looted for ammo/grenades. It's
        // only hidden right at the very end, briefly, before respawning
        // fresh elsewhere.
        b.mesh.visible = b.respawn > 0.6;
  ...
  private burst(x: number, y: number, z: number, n: number, spd: number, color: [number, number, number] = [1, 0.75, 0.44]) {
    let spawned = 0;
    for (let i = 0; i < this.pLife.length && spawned < n; i++) {
      ...
      this.pColor[i3] = color[0];
      this.pColor[i3 + 1] = color[1];
      this.pColor[i3 + 2] = color[2];
      spawned++;
    }
  }

  // Blood spray for a kill: darker, less bouncy/slower than an explosion
  // burst (blood doesn't launch upward like debris), and a duller red tone
  // rather than a bright, saturated one.
  private bloodBurst(x: number, y: number, z: number, n: number) {
    let spawned = 0;
    for (let i = 0; i < this.pLife.length && spawned < n; i++) {
      if (this.pLife[i] > 0) continue;
      this.pLife[i] = 0.3 + Math.random() * 0.35;
      const i3 = i * 3;
      this.pPos[i3] = x + (Math.random() - 0.5) * 0.3;
      this.pPos[i3 + 1] = y + Math.random() * 0.4;
      this.pPos[i3 + 2] = z + (Math.random() - 0.5) * 0.3;
      this.pVel[i3] = (Math.random() - 0.5) * 3.4;
      this.pVel[i3 + 1] = 1 + Math.random() * 2.2;
      this.pVel[i3 + 2] = (Math.random() - 0.5) * 3.4;
      const flicker = 0.82 + Math.random() * 0.18;
      this.pColor[i3] = 0.55 * flicker;
      this.pColor[i3 + 1] = 0.04 * flicker;
      this.pColor[i3 + 2] = 0.05 * flicker;
      spawned++;
    }
  }
  ...
  private renderVisuals(dt: number) {
    ...
    this.gunLight.intensity = 2.2 + this.muzzle * 34;
    ...
    if (flash) {
      ...
      m.opacity = Math.min(1, this.muzzle * 1.6);
      flash.scale.setScalar(1.0 + this.muzzle * 3.0);
    }
    ...
    // Gun view-model re-enabled now that draw() is a single unified render
    // pass (the old layer-1 second pass was what wiped the world, not the
    // gun mesh itself — see draw()).
    this.gun.visible = weaponsOn && this.player.weapon === "rifle";
    this.sniper.visible = weaponsOn && this.player.weapon === "sniper";
  ...
  private updateCamera(dt: number) {
    ...
    if (this.player.inCar >= 0) {
      const car = this.vehicles[this.player.inCar];
      // Camera position still sits behind the car's actual driving
      // direction (car.yaw) — you don't want the chase-cam itself
      // swinging wildly around the vehicle while cornering. But where the
      // camera POINTS (and therefore where the crosshair/gun aims) now
      // follows the player's own look input (player.yaw/pitch) instead of
      // being welded to car.yaw, so you can aim out the side/back window
      // for a drive-by while still steering and seeing where you're going.
      const fx = -Math.sin(car.yaw);
      const fz = -Math.cos(car.yaw);
      const dist = 8.4;
      const height = 3.6;
      ...
      const lookCp = Math.cos(this.player.pitch);
      _look.set(
        car.x + -Math.sin(this.player.yaw) * lookCp * 6,
        1.15 + Math.sin(this.player.pitch) * 6,
        car.z + -Math.cos(this.player.yaw) * lookCp * 6,
      );
