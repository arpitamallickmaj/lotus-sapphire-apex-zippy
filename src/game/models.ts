...
export function makeCar(color: number): THREE.Group {
  ...
  // Painted body panels use a glossy PBR material (not toon) — car paint
  // in the reference shots has a clear specular highlight/reflection
  // running along the body, which flat-shaded toon banding washes out.
  // Toon shading stays on trim/tires/rims below, where flat, saturated
  // color reads better than gloss.
  const bodyM = mat({ color, roughness: 0.28, metalness: 0.35, envMapIntensity: 1.3 });
  const bodyShadeM = mat({ color: shade(color, 0.72), roughness: 0.32, metalness: 0.32, envMapIntensity: 1.2 });
...
export function makeBike(color: number): THREE.Group {
  const g = new THREE.Group();
  const bodyM = mat({ color, roughness: 0.3, metalness: 0.5, envMapIntensity: 1.2 });
  const dark = toonMat({ color: 0x141416 });
  const trim = toonMat({ color: 0x3a3c42 });
  const rubber = toonMat({ color: 0x161618 });
  const rim = toonMat({ color: 0x9a9ca2 });
  const seatM = toonMat({ color: 0x1c1c1e });
  const lightF = toonMat({ color: 0xfff6dc, emissive: 0xffe7a0, emissiveIntensity: 2.0 });
  const lightR = toonMat({ color: 0xff3a2a, emissive: 0xaa1510, emissiveIntensity: 1.3 });

  // Main frame spine — a simple angled backbone running from the front
  // fork down to the rear axle, which is what actually reads as
  // "motorbike frame" from the side silhouette.
  const frame = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.12, 1.15), dark);
  frame.position.set(0, 0.62, -0.05);
  frame.rotation.x = 0.08;
  frame.castShadow = true;
  g.add(frame);

  const engine = new THREE.Mesh(new THREE.BoxGeometry(0.32, 0.28, 0.4), trim);
  engine.position.set(0, 0.44, -0.05);
  engine.castShadow = true;
  g.add(engine);

  const tank = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.22, 0.42), bodyM);
  tank.position.set(0, 0.72, -0.3);
  tank.castShadow = true;
  g.add(tank);

  const seat = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.08, 0.55), seatM);
  seat.position.set(0, 0.78, 0.15);
  g.add(seat);

  // Front fork + handlebars.
  const fork = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.62, 6), trim);
  fork.position.set(0, 0.5, -0.62);
  fork.rotation.x = -0.35;
  g.add(fork);
  const bars = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.04, 0.06), dark);
  bars.position.set(0, 0.82, -0.68);
  g.add(bars);

  // Headlight and taillight.
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.08, 8, 8), lightF);
  head.position.set(0, 0.68, -0.86);
  g.add(head);
  const tail = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.08, 0.04), lightR);
  tail.position.set(0, 0.72, 0.46);
  g.add(tail);

  const exhaust = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 0.5, 8), trim);
  exhaust.rotation.z = Math.PI / 2;
  exhaust.position.set(0.16, 0.32, 0.15);
  g.add(exhaust);

  const wheels: THREE.Group[] = [];
  const tireGeo = new THREE.CylinderGeometry(0.32, 0.32, 0.14, 20);
  tireGeo.rotateZ(Math.PI / 2);
  const rimGeo = new THREE.CylinderGeometry(0.17, 0.17, 0.15, 12);
  rimGeo.rotateZ(Math.PI / 2);
  for (const [x, z] of [
    [0, -0.85],
    [0, 0.58],
    ...
    wheelGroup.add(rimMesh);
    for (let i = 0; i < 5; i++) {
      const spoke = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.025, 0.025), trim);
      spoke.rotation.x = (i / 5) * Math.PI * 2;
      wheelGroup.add(spoke);
    }
    g.add(wheelGroup);
...
export function makeHuman(outfit: number, accent = 0x2a2c32): THREE.Group {
  ...
  // Slightly taller overall stance to match the raised eye height — scaling
  // around the group's own origin (the feet, at y=0) keeps characters
  // grounded while making them read as taller instead of stretching oddly.
